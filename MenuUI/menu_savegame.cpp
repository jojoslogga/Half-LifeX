...
=================
*/
static void UI_SaveGame_Close( void )
{
	int i;
	char saveshot[128];
	...
	{
		if( strlen( uiSaveGame.saveName[i] ))
		{
			sprintf( saveshot, "save\\%s.bmp", uiSaveGame.saveName[i] );
			PIC_Free( saveshot );
		}
	}
}
...
void UI_SaveGame_Menu( void )
{
	...
	uiSaveGame.menu.closeFunc = UI_SaveGame_Close;
